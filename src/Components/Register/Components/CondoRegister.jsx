        ...
        return (
            <div className='userRegister_mainView'>
                <section className='userRegister_section1'>
                    ...
                    <p className='register_opacity_appear'>¡Empecemos!</p>
                </section>

                <form action="#" method="post" className="residentRegister__form register_opacity_appear">
                <p className='register_opacity_appear'>Ingrese la información del Condominio</p>
                    <div className="register__inputBox">
                        <label htmlFor="">Nombre del condominio<span>*</span></label>
                        <input onChange={inputChange} id="name" type="text" className={`${errors.name ? 'errorInput' : ''}`} />
                        {errors.name && <p className='errors'>{errors.name}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">País<span>*</span></label>
                        <select onChange={inputChange} defaultValue='Argentina' id="country" name="País" className="register__inputBox">
                    ...
                    <div className="register__inputBox">
                        ...
                        <input onChange={inputChange} id="state" type="text" className={`${errors.state ? 'errorInput' : ''}`} />
                        {errors.state && <p className='errors'>{errors.state}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Ciudad/Provincia<span>*</span></label>
                        <input onChange={inputChange} id="city" type="text" className={`${errors.city ? 'errorInput' : ''}`} />
                        {errors.city && <p className='errors'>{errors.city}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Distrito/Comunidad<span>*</span></label>
                        <input onChange={inputChange} id="district" type="text" className={`${errors.district ? 'errorInput' : ''}`} />
                        {errors.district && <p className='errors'>{errors.district}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Teléfono del condominio<span>*</span></label>
                        <input onChange={inputChange} id="phone" type="number" max={5} min={0} className={`${errors.phone ? 'errorInput' : ''}`} />
                    ...
                    <div className="register_description register__inputBox">
                        ...
                        <textarea onChange={inputChange} id="placeDescription" type="text" maxLength={300} />
                    </div>


                    <section className='userRegister_section1'>
                        <p className='register_opacity_appear'>Ingrese la información del Administrador</p>
                    </section>


                    <div className="register__inputBox">
                        <label htmlFor="">Nombre del usuario<span>*</span></label>
                        <input onChange={inputChange} id="foreName" type="text" className={`${errors.foreName ? 'errorInput' : ''}`} />
                        {errors.foreName && <p className='errors'>{errors.foreName}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Apellido<span>*</span></label>
                        <input onChange={inputChange} id="lastName" type="text" className={`${errors.lastName ? 'errorInput' : ''}`} />
                        {errors.lastName && <p className='errors'>{errors.lastName}</p>}
                    </div>

                    <div className="register__inputBox register__select">
                        <select onChange={inputChange} defaultValue='dni' id="identification" name="Identificación" className="register__inputBox">
                            <option value="dni">Cédula de identidad/DNI</option>
                            <option value="pasaporte">Pasaporte</option>
                        </select>
                        <input onChange={inputChange} id="dni" type="text" placeholder='Identificación' className={`${errors.dni ? 'errorInput' : ''}`} />
                        {errors.dni && <p className='errors'>{errors.dni}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Teléfono<span>*</span></label>
                        <input onChange={inputChange} id="ownerPhone" type="text" className={`${errors.ownerPhone ? 'errorInput' : ''}`} />
                        {errors.ownerPhone && <p className='errors'>{errors.ownerPhone}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Email<span>*</span></label>
                        <input onChange={inputChange} id="ownerEmail" type="text" autoComplete="on" className={`${errors.ownerEmail ? 'errorInput' : ''}`} />
                        {errors.ownerEmail && <p className='errors'>{errors.ownerEmail}</p>}
                    </div>

                    <div className="register__inputBox">
                        <label htmlFor="">Contraseña<span>*</span></label>
                        <input onChange={inputChange} id="password" type="password" autoComplete="off" />
                        <p>Debe contener:</p>
                    ...
                    <div className="register__inputBox">
                        ...
                        <input onChange={inputChange} id="repeat_password" type="password" autoComplete="off" />
                        {errors.repeat_password && <p className='errors'>{errors.repeat_password}</p>}
                    </div>

                    {!isACondominiumOwner &&
                        <div className="register__checkBox">
                            <input onChange={inputChange} id="checkbox_confirm" type="checkbox" name="miCheckbox" />
                            <label for="miCheckbox">{`Soy Propietario o Administrador del Condominio`}</label>
                        </div>
                    }
